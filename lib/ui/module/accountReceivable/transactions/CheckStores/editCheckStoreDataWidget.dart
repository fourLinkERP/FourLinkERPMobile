...
import '../../../../../data/model/modules/module/accountreceivable/transactions/checkStores/checkStoreD.dart';
...
import '../../../../../service/module/accountReceivable/transactions/CheckStores/checkStoreHApiService.dart';
...
class EditCheckStoreDataWidget extends StatefulWidget {
  ...
  final CheckStoreH checkStoreH;

  @override
  State<EditCheckStoreDataWidget> createState() => _EditCheckStoreDataWidgetState();
}

class _EditCheckStoreDataWidgetState extends State<EditCheckStoreDataWidget> {

  final CheckStoreHApiService api = CheckStoreHApiService();
  int id = 0;
  ...
  void initState() {
    ...
    id = widget.checkStoreH.id!;
    _checkStoreSerialController.text = widget.checkStoreH.serial.toString();
    _checkStoreToDateController.text = DateFormat('yyyy-MM-dd').format(DateTime.parse(widget.checkStoreH.toDate!.toString()));
    selectedStoreValue = widget.checkStoreH.storeCode!;
    _notesController.text = widget.checkStoreH.notes!;
  ...
  fillCompos()
  {
    Future<List<CheckStoreD>> futureCheckStoreD = _checkStoreDApiService.getCheckStoreD(id).then((data) {
      checkStoreDLst = data;
      print(checkStoreDLst.length.toString());
      getCheckStoreData();
      return checkStoreDLst;
    }, onError: (e) {
      print(e);
    });
  ...
  void deleteInventoryRow(BuildContext context, int? id) async {
    ...
    if (confirmed!) {
      int indexToRemove = checkStoreDLst.indexWhere((p) => p.id == id);

      if (indexToRemove != -1) {
        _checkStoreDApiService.deleteCheckStoreD(context, id);

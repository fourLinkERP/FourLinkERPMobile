...
class AddStreamTypeScreen extends StatefulWidget {
  const AddStreamTypeScreen({Key? key}) : super(key: key);

  @override
  State<AddStreamTypeScreen> createState() => _AddStreamTypeScreenState();
}

class _AddStreamTypeScreenState extends State<AddStreamTypeScreen> {

  final StreamTypeApiService _api = StreamTypeApiService();
  final _codeController = TextEditingController();
  final _nameAraController = TextEditingController();
  final _nameEngController = TextEditingController();
  final _streamTypeUrlController = TextEditingController();
  ...
  void initState(){
    ...
    Future<NextSerial>  futureSerial = _nextSerialApiService.getNextSerial("TRC_TrainingCenterStreamTypes", "StreamTypeCode", " And CompanyCode=$companyCode And BranchCode=$branchCode" ).then((data) {
                    ...
                    Column(
                      children: [
                        ...
                        SizedBox(
                            width: 120,
                            height: 50,
                            child: Center(child: Text('streamUrl'.tr(), style: const TextStyle(fontWeight: FontWeight.bold,fontSize: 15)))
                        ),
                        const SizedBox(height: 15),
                        SizedBox(
                            width: 120,
                      ...
                      child: Column(
                        children: [
                          ...
                          SizedBox(
                            ...
                            width: 195,
                            child: TextFormField(
                              controller: _streamTypeUrlController,
                              keyboardType: TextInputType.text,
                              enabled: true,
                              decoration: InputDecoration(
                                filled: true,
  ...
  saveStreamType(BuildContext context) async{
    ...
    if(_streamTypeUrlController.text.isEmpty)
    {
      FN_showToast(context,'please_enter_url'.tr() ,Colors.black);
      return;
    }
    await _api.createStreamType(context,StreamType(
        streamTypeCode: _codeController.text ,
        streamTypeNameAra: _nameAraController.text ,
        streamTypeNameEng: _nameEngController.text ,
        streamTypeUrl: _streamTypeUrlController.text,

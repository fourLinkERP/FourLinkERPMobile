...
import 'package:fourlinkmobileapp/data/model/modules/module/accountreceivable/transactions/stock/stockReceive.dart';
import 'package:fourlinkmobileapp/data/model/modules/module/general/receipt/stockReceiptHeader.dart';
...
class PDFReceiveReceipt {
  static Future<File> generateStockReceive(ReceiveReceipt receipt) async {
        ...
        pw.MultiPage(
          ...
          build: (context) => [
            ...
            pw.Row(
                ...
                children: [
                  Center(
                    child:   Text(receipt.receiptHeader.companyStockTypeName.toString(),style: const TextStyle(fontSize: 16),textAlign: TextAlign.center),
                  )
            ...
            pw.Row(
                ...
                children: [
                  Center(
                    child:   Text(receipt.receiptHeader.stockTypeName.toString(),style: const TextStyle(fontSize: 16),textAlign: TextAlign.center),
                  )
                ]
            ),
            buildHeader2(receipt),

            buildInvoiceHeaderCells(receipt.receive),
            buildInvoice(receipt.receive),
            pw.Divider(),
            buildInvoiceNewTotal(receipt.receive),
  ...
  static pw.Widget buildHeader(ReceiveReceipt receipt) => pw.Column(
  ...
  static pw.Widget buildHeader2(ReceiveReceipt receipt) => pw.Column(
    crossAxisAlignment: pw.CrossAxisAlignment.start,
    children: [
      pw.Row(
        crossAxisAlignment: pw.CrossAxisAlignment.start,
  ...
  static pw.Widget buildReceiptHeader(StockReceiptHeader recpt) => pw.Column(
      ...
      Row(
          children:[
            Container(
                ...
                // flex: 2,
                child: Text(' ${recpt.companyName}',style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold))),
          ]),
      // Row(
      //     children:[
      //       Container(
      //         //flex: 2,
      //           child: Text(' ${recpt.co}',style: const TextStyle(fontSize: 20))),
      //     ]),
  ...
  static pw.Widget buildReceiptHeader2(StockReceiptHeader recpt) => pw.Column(
    crossAxisAlignment: pw.CrossAxisAlignment.end,
    children: [
      ...
      Row(
          children:[
            Container(
                ...
                //flex: 2,
                child: Container(
                    child: Text(' ${recpt.companyReceivePermissionNo}',style: const TextStyle(fontSize: 18),textAlign: TextAlign.left))),
  ...
  static pw.Widget buildCompanyLogo2(ReceiveReceipt receipt) => pw.Column(
  ...
  static pw.Widget buildInvoiceInfo(StockReceiveInfo info) {
  ...
  static pw.Widget buildInvoiceHeaderCells(StockReceive receive) {
    var itemNameTitle = langId==1? "الصنف":"Item";
    var qtyTitle = langId==1? "الكمية":"Quantity";
    var contractNoTitle = langId==1? "رقم العقد":"Contract No";
    var shipmentNoTitle = langId==1? "عدد الكراتين":"Shipment No";
    ...
    return pw.Table.fromTextArray(
      ...
      data: [],
      border: null,
      headerStyle: const pw.TextStyle(fontSize: 17),
  ...
  static pw.Widget buildInvoice(StockReceive receive) {
    ...
    final data = receive.items.map((item) {
      var shipmentNumber = item.shipmentNumber == null ? 0 :item.shipmentNumber;
      var shipmentWeightCount = item.shipmentWeightCount == null ? 0 : item.shipmentWeightCount;
      var contractNo = item.contractNo == null ? 0 : item.contractNo;
      var quantity = item.quantity == null ? 0 : item.quantity;
      var itemName =  item.description ;

      return [
        shipmentWeightCount.toString(),
    ...
    return pw.Table.fromTextArray(
      ...
      cellStyle: const pw.TextStyle(fontSize: 17),
      cellHeight: 10,
      cellAlignments: {
        0: pw.Alignment.center,
        1: pw.Alignment.center,
  ...
  static pw.Widget buildInvoiceNewTotal(StockReceive receive) {
    ...
    List<ReceiveTotal> receiveTotals= [] ;
    String rowsCountTitle = (langId==1? "عدد الاصناف   ":"Total Items  ");
    String rowsCountValue =receive.info.rowsCount.toString();
    String totalQtyTitle = (langId==1? "اجمالي الكمية":"Total Quantity  ");
    String totalQtyValue =receive.info.totalQty.toString();
    ReceiveTotal invoiceTotal1 =ReceiveTotal( totalField1: totalQtyValue,totalField2: totalQtyTitle,totalField3:rowsCountValue ,totalField4:rowsCountTitle );
    receiveTotals.add(invoiceTotal1);
    ...
    final data = receiveTotals.map((item) {
  ...
  static pw.Widget buildTotal(StockReceive receive) {
            ...
            child: pw.Column(
              ...
              children: [
                pw.Directionality(
                    textDirection: pw.TextDirection.rtl,
                    child: pw.Center(
                        child: pw.Text((langId==1? "عدد الاصناف   ":"Total Items  ") + receive.info.rowsCount.toString(),
                            style: const pw.TextStyle(
                              fontSize: 15, )))),
                pw.Directionality(
                    textDirection: pw.TextDirection.rtl,
                    child: pw.Center(
                        child: pw.Text((langId==1? "اجمالي الكمية":"Total Quantity  ") + receive.info.totalQty.toString(),
                            style: const pw.TextStyle(
                              fontSize: 15, )))),
                // pw.Directionality(
                //     textDirection: pw.TextDirection.rtl,
                ...
                //     textDirection: pw.TextDirection.rtl,
                //     child: pw.Center(
                //         child: pw.Text((langId==1? "الصافي ":"Net ") + receive.info.totalAfterVat.toString(),
                //             style: const pw.TextStyle(
                //                 fontSize: 15 )))),
                // pw.Divider(),
                // pw.Directionality(
                //     textDirection: pw.TextDirection.rtl,
                //     child: pw.Center(
                //         child: pw.Text((langId==1? " الصافي كتابة ":"Net Tafqeet ") + receive.info.tafqeetName.toString(),
                //             style: const pw.TextStyle(
                //                 fontSize: 15 )))),
  ...
  static pw.Widget buildFooter(ReceiveReceipt receipt , pw.MemoryImage barcodeImage) => pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.center,
      children: [
        pw.Divider(),
